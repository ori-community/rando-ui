<template>
  <div>
    ...
    <v-card v-if="loadedServerConfig" class="mb-4">
      <v-tabs centered grow color="primary" show-arrows>
        ...
        <v-tab-item class="pa-4">
          <p>Presets are collections of recommended randomizer options</p>
          <v-row>
            <v-col>
        ...
        <v-tab-item class="pa-4">
          <v-row>
            <v-col>
              <!-- Difficulty -->
              <h3>Difficulty</h3>
              <v-select v-model="seedgenConfig.difficulty" :items="availableDifficultiesArray" item-value="id" item-text="name">
                <template #item="{ item }">
                  <div>
              ...
              <div class="d-flex">
                <v-layout>
                  <v-checkbox v-model="glitchesActive" label="Glitches"></v-checkbox>
                  <v-btn :disabled="!glitchesActive" icon @click="glitchesConfig = !glitchesConfig"><v-icon>mdi-cog</v-icon></v-btn>
                </v-layout>
              </div>
              <wotw-seedgen-glitch-select v-if="glitchesConfig" v-model="seedgenConfig.glitches" class="mb-6" :glitches="availableGlitches" />
            </v-col>
            <v-col>
              <!-- Spawn -->
              <h3>Spawn</h3>
              <v-select v-model="seedgenConfig.spawn" :items="availableSpawns">
                <template #item="{ item }">
          ...
          <v-row>
            <v-col>
              <!-- Goal -->
              <h3 class="mb-3">Goal</h3>
              <wotw-seedgen-goal-select v-model="seedgenConfig.goals" @goalSelected="goalSelected" :goals="availableGoals" :bingoTypes="availableBingoTypes" />
            </v-col>
          </v-row>
        </v-tab-item>
        <v-tab-item class="pa-4">
          <v-row>
            <v-col>
              <wotw-seedgen-header-group :group="availableHeaderGroups['QOL']" />
            </v-col>
          </v-row>
          <v-row>
            <!-- <v-col v-for="(group, id) in listedHeaderGroups" :key="id">
              <wotw-seedgen-header-group :group="availableHeaderGroups[id]" />
            </v-col> -->
          </v-row>
        </v-tab-item>
        <v-tab-item class="pa-4">
          <v-row>
            <v-col cols="12" md="6">
              ...
              <wotw-seedgen-flag-checkbox
                ...
                persistent-hint
              />
              <wotw-seedgen-flag-checkbox v-model="seedgenConfig.flags" flag="--race" hint="Useful for races" label="Disable Spoilers" persistent-hint />
            </v-col>
            <v-col cols="12" md="6">
              <wotw-seedgen-flag-checkbox
                v-model="seedgenConfig.flags"
    ...
    <v-dialog v-model="showBingoHeaderWarningDialog" max-width="700">
      <v-card>
        <v-btn class="close-button" color="background lighten-5" icon @click="showBingoHeaderWarningDialog = false">
          <v-icon>mdi-close</v-icon>
        </v-btn>
        <v-card-title>No Bingo header selected</v-card-title>
        <v-card-text>
          You are about to create a Bingo game but have not selected the Bingo header. We recommend that you enable the Bingo header so you can warp to credits
          after you completed the goal.
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
<script>
  ...
  // import tempHeaders from '~/assets/seedgen/tempHeaders.yaml'
  import headerGroups from '~/assets/seedgen/headerGroups.yaml'
  import spawns from '~/assets/seedgen/spawns.yaml'
  import bingoTypes from '~/assets/seedgen/bingoTypes.yaml'
  ...
  export default {
    ...
    watch: {
      ...
      glitchesActive(value) {
        if (!value) {
          this.glitchesConfig = false
        }
      },
    },
    async mounted() {
    ...
    methods: {
      ...
      goalSelected(goal, active) {},
      toggleBingoGoal(bingoType) {
        this.selectedBingo = bingoType.seedGenValue
        console.log(this.selectedBingo)
      },
      resetConfig() {
        this.seedgenConfig = generateNewSeedgenConfig()
        this.glitchesConfig = false
        this.glitchesActive = false
      ...
      async generate(ignoreMissingBingoHeader = false) {
        ...
        if (!this.glitchesActive) {
          this.seedgenConfig.glitches = []
        }
        console.log(this.seedgenConfig.glitches)
        ...
        try {
          ...
          if (!this.createOnlineGame) {
            if (result.hasMultipleSeeds) {
              ...
              confettiFromElement((this.launchAfterGenerating ? this.$refs.launchButton : this.$refs.createOrDownloadButton).$el, {
