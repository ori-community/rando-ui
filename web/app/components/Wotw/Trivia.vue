<template>
  <v-card class="trivia" @click="nextTrivia()">
    ...
    <img class="ori-think" src="@shared/images/ori_think.png" alt="">
  </v-card>
</template>
...
<script lang="ts" setup>
  ...
  const triviaIndex = ref(0)
  const triviaList = computed(() => shuffle([
  ...
  const currentTriviaHtml = computed(() => triviaList.value[triviaIndex.value] ?? '???')

  function nextTrivia() {
    triviaIndex.value = triviaIndex.value >= triviaList.value.length - 1
      ? 0
      : triviaIndex.value + 1
  }
</script>
