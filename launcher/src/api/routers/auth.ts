  ...
  startOAuthFlow: publicProcedure
    ...
    .query(async ({input}) => {
      if (app.isDefaultProtocolClient(DEFAULT_PROTOCOL) && !input.forceWindowLogin) {
        await shell.openExternal(`${input.apiBaseUrl}/login?redirect=${DEFAULT_PROTOCOL}:///auth/callback`)
      } else {
        ...
        await loginWindow.loadURL(`${input.apiBaseUrl}/login?redirect=${DEFAULT_PROTOCOL}:///auth/callback`)
