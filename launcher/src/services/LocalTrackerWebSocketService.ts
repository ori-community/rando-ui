...
import { RandoIPCService } from './RandoIPCService'
import {
  AuthenticateMessage,
  RequestFullUpdate,
...
export class LocalTrackerWebSocketService {
  ...
  static async reportCurrentTimerState() {
    const { in_game_time: inGameTime, async_loading_time: asyncLoadingTime, timer_should_run: timerShouldRun } = await RandoIPCService.request('timer.get_timer_state') as {
      in_game_time: number,
