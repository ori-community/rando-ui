<template>
  <div class="d-flex flex-column flex-grow-1 relative">
    ...
    <wotw-map ref="map" class="flex-grow-1 flex-shrink-1">
      <k-layer>
        <k-line
          ...
          :key="line.elementDisplayIndex"
          :config="line"
          @click="selectDisplayedObject(line.name)"
          @mouseout="displayElementMouseOut(line.name)"
          @mouseover="displayElementMouseOver(line.name)"
        />

        <k-circle
          v-for="circle in displayedCircles"
          :key="circle.elementDisplayIndex"
          :config="circle"
          @click="selectDisplayedObject(circle.name)"
          @mouseout="displayElementMouseOut(circle.name)"
    ...
    <v-btn
      ...
      icon
      class="selection-info-right-toggle"
      ...
      <v-card v-if="selectionInfoOpen" class="selection-info-right pa-3">
        ...
        <template v-if="selectionInfo.typeDescription != 'connection'">
          ...
          <table class="selection-info-table selection-info-table-copyable">
            <tbody>
              <tr
                v-for="connection in selectionInfo.nodeConnections"
                :key="connection.nodeName"
                :class="{ 'indirect-connection': connection.toNode }"
              >
              <td>{{ capitalCase(connection.typeDescription) }}:</td>
              <td>
                <a
                @click="selectDisplayedObject(connection.connectionName)"
            ...
            <td>
              <a
              @click="selectDisplayedObject(connection.nodeName)"
          ...
          <table class="selection-info-table-copyable selection-info-table">
            <tbody>
              <tr>
                ...
                <td>
                  <a
                    @click="selectDisplayedObject(selectedObject.start)"
              ...
              <tr>
                ...
                <td>
                  <a
                  @click="selectDisplayedObject(selectedObject.end)"
