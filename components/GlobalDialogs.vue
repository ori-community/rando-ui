  ...
  <div>
      ...
      <v-card>
        <div class="pa-6">
          ...
          <div class="d-flex justify-end">
            <v-btn text depressed @click="currentSupportBundleName = null"> Close </v-btn>
            <v-btn depressed color="accent" class="ml-2" @click="showSupportBundleInExplorer"> Show in Explorer </v-btn>
          </div>
        </div>
    ...
    <v-dialog v-model="showUpdateAvailableDialog" max-width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          An update for the randomizer is ready to be downloaded and installed. We recommend to always play on the
          latest version.
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn text class="mr-1" @click="showUpdateAvailableDialog = false"> Cancel </v-btn>
          <v-btn text class="mr-1" @click="forceLaunch"> Launch anyway </v-btn>
          <v-btn depressed color="accent" @click="downloadAndInstallUpdate"> Install update </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showTraceMap" fullscreen>
      <v-card color="black">
        <wotw-trace-map />
      </v-card>
    </v-dialog>
...
<script>
  ...
  export default {
    name: 'GlobalDialogs',
    computed: {
      ...mapGetters('version', ['latestVisibleVersion']),
      ...
      showTraceMap: {
        set(value) {
          this.$store.commit('electron/setShowTraceMap', value)
        },
        get() {
          return this.$store.state.electron.showTraceMap
        },
      },
      traceMapSource: {
        set(value) {
          this.$store.commit('electron/setTraceMapSource', value)
        },
        get() {
          return this.$store.state.electron.traceMapSource
        },
      },
    },
    methods: {
