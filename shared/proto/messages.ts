...
/** 102 */
export interface TrackerTagsUpdate {
  $type: "Proto.TrackerTagsUpdate";
  tags: string[];
}
...
function createBaseTrackerTagsUpdate(): TrackerTagsUpdate {
  return { $type: "Proto.TrackerTagsUpdate", tags: [] };
}

export const TrackerTagsUpdate: MessageFns<TrackerTagsUpdate, "Proto.TrackerTagsUpdate"> = {
  $type: "Proto.TrackerTagsUpdate" as const,

  encode(message: TrackerTagsUpdate, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): TrackerTagsUpdate {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseTrackerTagsUpdate();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): TrackerTagsUpdate {
    return {
      $type: TrackerTagsUpdate.$type,
      tags: globalThis.Array.isArray(object?.tags) ? object.tags.map((e: any) => globalThis.String(e)) : [],
    };
  },

  toJSON(message: TrackerTagsUpdate): unknown {
    const obj: any = {};
...
messageTypeRegistry.set(TrackerTagsUpdate.$type, TrackerTagsUpdate);
...
export const NormalGameHandlerState: MessageFns<NormalGameHandlerState, "Proto.NormalGameHandlerState"> = {
  ...
  encode(message: NormalGameHandlerState, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    globalThis.Object.entries(message.playerInGameTimes).forEach(([key, value]: [string, number]) => {
    ...
    globalThis.Object.entries(message.playerFinishedTimes).forEach(([key, value]: [string, number]) => {
    ...
    globalThis.Object.entries(message.worldFinishedTimes).forEach(([key, value]: [string, number]) => {
    ...
    globalThis.Object.entries(message.universeFinishedTimes).forEach(([key, value]: [string, number]) => {
  ...
  fromJSON(object: any): NormalGameHandlerState {
    return {
      ...
      playerInGameTimes: isObject(object.playerInGameTimes)
        ? (globalThis.Object.entries(object.playerInGameTimes) as [string, any][]).reduce(
      ...
      playerFinishedTimes: isObject(object.playerFinishedTimes)
        ? (globalThis.Object.entries(object.playerFinishedTimes) as [string, any][]).reduce(
          (acc: { [key: number]: number }, [key, value]: [string, any]) => {
            acc[globalThis.Number(key)] = globalThis.Number(value);
            return acc;
          },
      ...
      worldFinishedTimes: isObject(object.worldFinishedTimes)
        ? (globalThis.Object.entries(object.worldFinishedTimes) as [string, any][]).reduce(
          (acc: { [key: number]: number }, [key, value]: [string, any]) => {
            acc[globalThis.Number(key)] = globalThis.Number(value);
            return acc;
          },
      ...
      universeFinishedTimes: isObject(object.universeFinishedTimes)
        ? (globalThis.Object.entries(object.universeFinishedTimes) as [string, any][]).reduce(
          (acc: { [key: number]: number }, [key, value]: [string, any]) => {
            acc[globalThis.Number(key)] = globalThis.Number(value);
            return acc;
          },
  ...
  toJSON(message: NormalGameHandlerState): unknown {
    ...
    if (message.playerInGameTimes) {
      const entries = globalThis.Object.entries(message.playerInGameTimes) as [string, number][];
      if (entries.length > 0) {
    ...
    if (message.playerFinishedTimes) {
      const entries = globalThis.Object.entries(message.playerFinishedTimes) as [string, number][];
      if (entries.length > 0) {
    ...
    if (message.worldFinishedTimes) {
      const entries = globalThis.Object.entries(message.worldFinishedTimes) as [string, number][];
      if (entries.length > 0) {
    ...
    if (message.universeFinishedTimes) {
      const entries = globalThis.Object.entries(message.universeFinishedTimes) as [string, number][];
      if (entries.length > 0) {
